...
void TestMagic()
{
  const server::MessageMagic magic{
    .id = 7,
    .length = 29};

  // Test encoding of the magic.
  const auto encoded_magic = server::encode_message_magic(magic);
  assert(encoded_magic == 0x8D06CD01);

  // Test decoding of the magic.
  const auto decoded_magic = server::decode_message_magic(encoded_magic);
